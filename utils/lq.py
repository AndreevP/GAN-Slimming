...
class _LearnableFakeQuantizePerTensorOp(torch.autograd.Function):
    ...
    def forward(ctx, X, scale, zero_point, q_min, q_max, grad_factor):
        ...
        ctx.other = q_min, q_max, grad_factor, X_fq
        return X_fq
    ...
    def backward(ctx, dY):
        ...
        device = X.device
        dY = dY.to(device)
        q_min, q_max, grad_factor, X_fq = ctx.other
...
class _LearnableFakeQuantize(nn.Module):
    ...
    * attr: `learning_enabled` defines the flag for enabling backpropagation for scale and zero point.
    ...
    def forward(self, X):
        ...
        if (self.init):
            self.scale = Parameter(torch.tensor([1.] * len(_scale)))
            self.zero_point = Parameter(torch.tensor([0.] * len(_zero_point)))
            return X
